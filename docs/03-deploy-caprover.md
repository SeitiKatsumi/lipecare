...
O serviço `lipecare-api-production` deve usar `captain-definition.api`, executar as migrations no início do contêiner e receber `DATABASE_URL` e `WEB_URL` pelo CapRover. A variável `LIPECARE_SEED_DEMO` deve ser habilitada somente no primeiro deploy de um banco vazio e removida em seguida.
